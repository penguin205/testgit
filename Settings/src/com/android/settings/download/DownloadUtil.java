	...
	public void start() {
		new AsyncTask<Void, Void, Void>() {
			...
			protected void onPostExecute(Void result) {
				fileSize = mDownloadHttpTool.getFileSize();            //ready中初始化
				downloadedSize = mDownloadHttpTool.getCompeleteSize(); //ready中初始化 
				Log.w("Tag", "downloadedSize::" + downloadedSize);
				if (onDownloadListener != null) {
					onDownloadListener.downloadStart(fileSize, downloadedSize);
				}
				if(downloadedSize < fileSize) {
