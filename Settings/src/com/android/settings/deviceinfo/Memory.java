...
public class Memory extends SettingsPreferenceFragment {
	...
	private static final int SD_DLG_CONFIRM_UNMOUNT = 1;
	private static final int USB_DLG_CONFIRM_UNMOUNT = 2;
	private static final int SD_DLG_ERROR_UNMOUNT = 3;
	private static final int USB_DLG_ERROR_UNMOUNT = 4;
	...
	public Dialog onCreateDialog(int id) {
		...
		case USB_DLG_CONFIRM_UNMOUNT:
			...
			.setPositiveButton(R.string.dlg_ok, new DialogInterface.OnClickListener() {
				...
				.create();
		case SD_DLG_ERROR_UNMOUNT:
			return new AlertDialog.Builder(getActivity())
			.setTitle(R.string.dlg_error_unmount_title)
			.setNeutralButton(R.string.dlg_ok, null)
			.setMessage(R.string.dlg_error_unmount_text)
			.create();
		case USB_DLG_ERROR_UNMOUNT:
			return new AlertDialog.Builder(getActivity())
	...
	private void doUnmount() {
		...
		if("/storage/extsd".equals(sClickedMountPoint)){
			...
			try {
				...
				showDialogInner(SD_DLG_ERROR_UNMOUNT);
			}
		}else{
			Toast.makeText(getActivity(), R.string.usb_unmount_inform_text, Toast.LENGTH_SHORT).show();
			try {
				sLastClickedMountToggle.setEnabled(false);
				sLastClickedMountToggle.setTitle(getString(R.string.usb_ejecting_title));
				sLastClickedMountToggle.setSummary(getString(R.string.usb_ejecting_summary));
				mountService.unmountVolume(sClickedMountPoint, true, false);
			} catch (RemoteException e) {
				// Informative dialog to user that unmount failed.
				showDialogInner(USB_DLG_ERROR_UNMOUNT);
			}
		}
	}
	...
	private void unmount() {
		...
		try {
			if (hasAppsAccessingStorage()) {
				...
				if("/storage/extsd".equals(sClickedMountPoint)){
					showDialogInner(SD_DLG_CONFIRM_UNMOUNT);
				} else {
					showDialogInner(USB_DLG_CONFIRM_UNMOUNT); 
				}
			} else {
				doUnmount();
			}
